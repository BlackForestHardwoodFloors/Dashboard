...
// ========================================
// SAMPLE DATA
// (moved to ./sampleClients so Calendar + Clients stay in sync)
// ========================================
...
export default function ClientsPage({ onNavigate }: { onNavigate?: (page: string) => void }) {
  ...
  // If the user came from Calendar (clicked an appointment), pre-select the matching client in this demo page.
  useEffect(() => {
    const pickedName = sessionStorage.getItem('br_calendar_selected_contact_name') || '';
    const pickedPhone = sessionStorage.getItem('br_calendar_selected_contact_phone') || '';
    const pickedId = sessionStorage.getItem('br_calendar_selected_contact_id') || '';

    if (!pickedName && !pickedPhone && !pickedId) return;

    const normalize = (v: string) => (v || '').toLowerCase().replace(/\s+/g, ' ').trim();
    const digits = (v: string) => (v || '').replace(/\D/g, '');

    const nameNorm = normalize(pickedName);
    const phoneNorm = digits(pickedPhone);

    let found = null as any;

    // 1) Strongest match: by id from calendar click
    if (pickedId) {
      found = sampleClients.find((c: any) => String(c.id) === String(pickedId));
    }

    // 2) Fallback: match by primary contact name/phone (older behavior)
    if (!found) {
      found = sampleClients.find((c: any) => {
        const pc = c.primaryContact || {};
        const full = normalize([pc.firstName, pc.lastName].filter(Boolean).join(' '));
        const account = normalize(c.accountName || '');
        const pcPhone = digits(pc.phone || (pc.phones && pc.phones[0]) || '');
        const byName = nameNorm && (full === nameNorm || account.includes(nameNorm) || nameNorm.includes(full));
        const byPhone = phoneNorm && pcPhone && phoneNorm === pcPhone;
    ...
    // Clean up so refreshes don't keep forcing a selection
    sessionStorage.removeItem('br_calendar_selected_contact_name');
    sessionStorage.removeItem('br_calendar_selected_contact_phone');
    sessionStorage.removeItem('br_calendar_selected_contact_id');
    sessionStorage.removeItem('br_calendar_selected_appointment_id');
  }, []);
