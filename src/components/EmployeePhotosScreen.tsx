...
export function EmployeePhotosScreen({
  ...
  const handleTouchMove = useCallback((e: React.TouchEvent) => {
    ...
    if (Math.abs(diffX) > diffY) {
      setSwipeOffset(-diffX * 0.3); // Reduced movement for feel
  ...
  const handleTouchEnd = useCallback(() => {
    ...
    if (Math.abs(distanceX) > minSwipeDistance) {
      if (distanceX > 0 && fullscreenIndex < filteredPhotos.length - 1) {
        ...
        setSelectedPhoto(filteredPhotos[fullscreenIndex + 1]);
  ...
  const handlePullEnd = () => {
    if (pullDistance > 50 && !isRefreshing) {
      ...
      // Simulate refresh - in production, refetch photos
      setTimeout(() => {
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '12px' }}>
          ...
          <h1 style={{ color: colors.text, fontSize: '20px', fontWeight: '700', margin: 0, flex: 1, textAlign: onClose ? 'center' : 'left' }}>
            Job Photos
          </h1>
          <button
            onClick={onTakePhoto}
          ...
          Object.entries(groupedPhotos).map(([date, datePhotos]) => (
            <div key={date} style={{ marginBottom: '8px' }}>
              ...
              <div style={{
                ...
                <p style={{ color: colors.textSecondary, fontSize: '13px', fontWeight: '600', margin: 0 }}>
                  {date}
                </p>
              </div>
      ...
      {selectedPhoto && (
        <div style={{
          ...
          <div style={{
            ...
            <div>
              <p style={{ color: colors.text, fontSize: '16px', fontWeight: '600', margin: '0 0 4px 0' }}>
                {selectedPhoto.jobName}
              </p>
              <p style={{ color: colors.textSecondary, fontSize: '13px', margin: 0 }}>
          ...
          >
            ...
            {fullscreenIndex > 0 && (
              <div style={{
                ...
                opacity: swipeOffset > 20 ? 0.8 : 0.3,
          ...
          <div style={{
            ...
            <div style={{ display: 'flex', gap: '8px', justifyContent: 'center' }}>
              {filteredPhotos.slice(
                Math.max(0, fullscreenIndex - 4),
                Math.min(filteredPhotos.length, fullscreenIndex + 5)
              ).map((photo, i) => {
                const actualIndex = Math.max(0, fullscreenIndex - 4) + i;
                return (
                  <div
                    key={photo.id}
                    onClick={() => {
